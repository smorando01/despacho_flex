...
// Determina si el código pertenece al patrón de Colecta y devuelve el ID numérico
...
// Analiza un código pensando en Flex (QR o numérico largo, excepto 11 dígitos)
...
try {
    ...
    if (!$session) {
        ...
        echo json_encode([
            'success' => false,
            'message' => 'No hay ningún despacho abierto. Iniciá uno antes de escanear.'
        ]);
        exit;
    }
    ...
    if ($courier === 'FLEX') {
        if ($colectaId !== null) {
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Este código es de Colecta, no de Flex'
            ]);
            exit;
        }
        ...
        if ($colectaId !== null) {
            ...
            echo json_encode([
                'success' => false,
                'message' => 'Este código es de Flex, no de Colecta'
            ]);
            exit;
        } else {
            $codigo_final = $raw;
        ...
        echo json_encode([
            'success' => false,
            'message' => 'Courier de la sesión no soportado.'
        ]);
        exit;
    }
